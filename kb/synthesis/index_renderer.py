...
_INDEX_SYSTEM_PROMPT = (
    ...
    "Linking: mention a page with `[[type:slug|Title]]` where it genuinely "
    "helps a reader go deeper on something you just said — the `type` and "
    "`slug` are given for every page in the corpus below, and `Title` is what "
    "the reader sees. That is a handful of links inside prose, not a bullet "
    "per page. A paragraph that is mostly links has become the directory this "
    "page must not be.\n\n"
    "**Always the `type:slug` form, never bare `[[Title]]`.** A bare title is "
    "not resolvable: an agent reading this page cannot turn `[[research-os]]` "
    "into the page `repo/research_os`, and `[[Wiki Synthesis Pipeline "
    "(prbe-knowledge)]]` into `feature/prbe_knowledge-wiki_synthesis` is not "
    "guessable at all. This is the first page every person and every agent "
    "reads, so a link they cannot follow is the most expensive kind to write. "
    "The typed form also becomes a real edge in the link graph; the bare form "
    "is recorded as a dangling reference and connects nothing.\n\n"
