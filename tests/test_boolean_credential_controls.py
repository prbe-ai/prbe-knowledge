...
The vendored gate REPORTS findings (`has_findings`) instead of refusing; the
manual-upload boundary (kb/ingestion_app.py) is what refuses, and
tests/test_manual_upload_secret_boundary.py pins that.
"""
...
from engine.ingest._credential_gate import inspect_bytes
...
def test_serialized_boolean_report_keeps_original_bytes(value):
    ...
    assert not inspect_bytes(raw.encode()).has_findings
...
def test_vendor_credential_key_with_boolean_value_is_still_removed():
    ...
    assert inspect_bytes(json.dumps({key: True}).encode()).has_findings
