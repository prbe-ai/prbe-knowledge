...
from engine.shared.constants import AUTO_MERGE_RETRY_SECONDS, JEV_BREAKER_FAILURES
...
async def world(live_db, monkeypatch):
    ...
    monkeypatch.setattr(jev_judge, "MERGE_BREAKER", jev._Breaker())
    ...
    async def lock_like_an_embedding_write(self, conn, node_id):
        ...
        await conn.execute("UPDATE graph_nodes SET updated_at = updated_at WHERE node_id = $1", node_id)
    ...
    return types.SimpleNamespace(fake=fake, worker=PostWriteWorker(concurrency=1, execute_high_confidence=True))
...
async def test_jev_outage_keeps_the_node_queued_and_retries_it(world):
    ...
    (row,) = await fetch(
        "SELECT analyzer_status, locked_until > NOW() + interval '4 minutes' AS delayed FROM node_post_write_queue"
    )
    status = json.loads(row["analyzer_status"])["auto_merge"]
    assert (status["status"], status["attempts"]) == ("deferred", 1)
    assert row["delayed"]  # AUTO_MERGE_RETRY_SECONDS x 1 attempt
    assert AUTO_MERGE_RETRY_SECONDS >= 300
...
async def test_open_breaker_defers_without_spending_an_attempt(world):
    await ingest(make_person("ada@example.com", {"name": "Ada Lovelace", "email": "ada@example.com"}))
    await drain(world.worker)
    for _ in range(JEV_BREAKER_FAILURES):
        jev_judge.MERGE_BREAKER.failure()

    await ingest(make_person("ada-gh", {"name": "Ada Lovelace", "login": "ada-gh", "email": "ada@example.com"}))
    await drain(world.worker)

    assert world.fake.requests == []  # nothing was sent
    (row,) = await fetch("SELECT analyzer_status FROM node_post_write_queue")
    status = json.loads(row["analyzer_status"])["auto_merge"]
