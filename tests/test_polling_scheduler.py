...
Tests use a stub poller that registers as ``SourceSystem.MANUAL_UPLOAD`` for
...
def _register_stub_poller():
    ...
    _StubPoller.reset()
    register_poller(SourceSystem.MANUAL_UPLOAD, _StubPoller)
    yield _StubPoller
    polling_base._REGISTRY.pop(SourceSystem.MANUAL_UPLOAD, None)
...
async def test_tick_one_runs_registered_poller_and_advances_cursor(
    ...
    assert documents_seen == [(_TENANT, SourceSystem.MANUAL_UPLOAD, [{"id": "doc1"}])]

    loaded = await load_cursor(
        customer_id=_TENANT, source=SourceSystem.MANUAL_UPLOAD, resource_id="C123"
    )
...
async def test_tick_one_stamps_error_on_poller_raised(
    ...
    polling_base._REGISTRY[SourceSystem.MANUAL_UPLOAD] = _ExplodingPoller
