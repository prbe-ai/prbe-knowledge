...
|------|------|----------|----------|------------|------------|---------|
| `shared/embeddings.py` | 216 | `OpenAIEmbedder.__init__` (client constructed); `_embed_once` line 229 calls `embeddings.create` | OpenAI | `text-embedding-3-large` (`shared.constants.EMBEDDING_MODEL`) | embedding (batched) | Primary embedder for the query path until Stage 4 cutover; powers pgvector similarity search |
| `shared/embeddings.py` | 317 | `GeminiEmbedder._ensure_client` + `_embed_subbatch` line 398 (`client.aio.models.embed_content`) | Google | `gemini-embedding-2-preview` (`shared.constants.EMBEDDING_V2_MODEL`) | embedding (batched, parallel sub-batches) | Ingest-side dual-write embedder; will be primary post-Stage-4. Asymmetric prefixing (`title: ... | text: ...` for docs vs `task: search result | query: ...` for queries) |
...
| `services/synthesis/providers.py` | 234 | `_gemini_call_json` (used by `_GeminiTriage.triage` line 287) | Google | `gemini-flash-lite-preview` / `gemini-flash-lite-preview` variants | completion + `response_schema` (structured JSON) | Alternate triage backend; flippable via `WIKI_TRIAGE_MODEL` constant |
...
| `services/synthesis/agent_compactor.py` | 110 | `compact_agent_conversation` | Google | `WIKI_AGENT_COMPACTOR_MODEL` (gemini-flash-lite-preview) | completion (free-text summary) | Wiki-agent loop conversation compactor (fires when context >60% of window) |
