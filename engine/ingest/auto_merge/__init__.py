"""Entity auto-merge: model-judged dedup for graph_nodes.
...
For each candidate (label, canonical_id), the analyzer:
  1. Finds nearby entities via pg_trgm on LOWER(canonical_id) + LOWER(properties->>'name')
     and a nearest-neighbour search on graph_nodes.embedding (when populated)
  2. Filters candidates with conflicting stable properties (mismatched email, etc.)
  3. Asks a judge which survivor, if any, is the same entity: Jev by default
     (one Choice question, `jev_judge.py`); Cerebras gpt-oss-120b with a
     `response_format=AutoMergeVerdict` schema as the code-level rollback
     (`AUTO_MERGE_JUDGE`)
  4. On confidence='high': posts to the existing entity-clusters merge txn
     (a Person needs a shared email or login too, else it is a suggestion)
