...
then POSTs to this service's `/webhooks/{source}` with X-Internal-Key
...
def _verify_internal_key(request: Request) -> None:
    ...
    if expected is None or not expected.get_secret_value():
        raise HTTPException(
            status_code=503, detail="ADMIN_API_KEY (X-Internal-Key) not configured"
        )
    presented = request.headers.get("x-internal-key") or request.headers.get(
        "x-admin-key"
    )
    if not presented or not hmac.compare_digest(
        ...
        raise HTTPException(
            status_code=401, detail="missing or invalid X-Internal-Key"
        )
...
async def webhook(
    ...
    Trusts X-Internal-Key + X-Prbe-Customer; does NOT verify the source
