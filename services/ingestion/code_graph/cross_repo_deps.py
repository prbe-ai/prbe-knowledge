...
_CLASSIFY_SYSTEM_PROMPT = (
    ...
    "**Direction matters.** The edge is `source -> target`: the SOURCE "
    "depends on the TARGET. So the question is: 'does the SOURCE repo's "
    "runtime, build-time, or deployment behavior depend on the TARGET? "
    "If the TARGET disappeared, would the SOURCE's code break or build "
    "wrong?'\n\n"
    "If a code reference describes who CALLS the source (not who the "
    "source calls), the arrow points the wrong way and the match is "
    "COINCIDENCE — the reverse edge will be picked up when the other "
    "repo is scanned. The source's code mentioning the target is NOT "
    "enough; the source must actually consume something from the target.\n\n"
    "REAL examples (source genuinely depends on target):\n"
    "  - HTTP client base URL pointing at <target>.internal / "
    "<target>.fly.dev / <target>.com (source CALLS target)\n"
    "  - Env var providing the target's hostname or URL that the source "
    "READS to make outbound calls (e.g. TARGET_URL, BACKEND_URL=...)\n"
    "  - Import / require of a package, type, or SDK owned by the target\n"
    "  - GitHub Actions `uses: <org>/<target>@...` or workflow_dispatch "
    "to the target\n"
    "  - Cross-repo protocol/schema/contract that source CONSUMES at "
    "compile time\n"
    "  - Producer-consumer queue coupling where source publishes to a "
    "queue the target reads, OR source consumes a queue the target "
    "writes\n\n"
    "COINCIDENCE examples (look like references but are NOT "
    "source -> target dependencies):\n"
    "  - User-facing setup/install instructions naming the target "
    "(CLI commands like `plugin marketplace add <org>/<target>` shown "
    "in onboarding modals — that's the USER's dependency, not this code's)\n"
    "  - Filesystem paths the TARGET writes/manages (e.g. "
    "`~/.<cli>/plugins/cache/<org>/<target>/...`)\n"
    ...
    "  - Substring match inside a longer identifier\n"
    "  - Vendored / generated content where the mention is irrelevant\n"
    "  - **Wrong-direction references** — source's code documents that "
    "TARGET calls / consumes / depends on source. The arrow points the "
    "wrong way. Concrete patterns:\n"
    "    * `INTERNAL_<TARGET>_API_KEY — value <target> presents` "
    "(target is calling source; source just validates an incoming "
    "token; if target disappeared, the check never fires)\n"
    "    * `used by the <target> watcher`, `for the <target> daemon`, "
    "`routes <target> calls`, `the <target> consumer` (target consumes "
    "source; source's routes still work without target)\n"
    "    * `Middleware implements auth logic based on <target> callers` "
    "(target is the caller)\n"
    "    * `mirrors prbe-<other>'s pattern` (shared design idea, not a "
    "runtime dep)\n"
    "    * Comments documenting that the OTHER repo will hit this "
    "endpoint or call this function\n\n"
    "Final litmus test: 'If the target repo were DELETED from the org, "
    "would the SOURCE's code stop working or build wrong?' If yes -> "
    "REAL. If the source would still function (perhaps with unused "
    "config, stale comments, or never-fired auth checks) -> COINCIDENCE.\n\n"
