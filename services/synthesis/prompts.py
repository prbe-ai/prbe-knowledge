...
def _wiki_agent_system(now: datetime) -> str:
    ...
    return (
        ...
        "**Stable overviews only.** Pages describe what something IS, "
        "not what just happened. Never add 'Recent Activity', 'Recent "
        "PRs', 'Recent Architectural Changes', or any other "
        "time-bounded section to a page body — those go stale fast "
        "and search covers them. If the only meaningful update from a "
        "drain is 'PR #123 landed', skip the event. The page body "
        "should read the same in 6 months as it does today.\n\n"
        "**Workflow per drain:**\n"
...
def build_github_repo_subtask_prompt(
    *,
    ...
    return (
        ...
        f"**Augment, don't replace.** When updating repo/{repo_slug}, "
        "your additions should be STABLE OVERVIEW content — what this "
        "repo IS, not what just happened to it. Specifically:\n"
        "  - flesh out the architectural overview: key modules, public "
        "surfaces, data flow, primary owners\n"
        "  - identify per-file owners + reviewers (from PR review "
        "data) and link them via [[person:login]]\n"
        "  - call out durable runbook steps if PRs reference "
        "deploy/operate/recover procedures\n"
        "  - do NOT add 'Recent Activity', 'Recent Architectural "
        "Changes', or any time-bounded section — those go stale fast "
        "and search covers them. The timeline table (`record_timeline`) "
        "is the right place for per-event audit trail; the page body "
        "should read the same in 6 months as it does today.\n\n"
